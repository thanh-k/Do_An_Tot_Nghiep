...
function StaffManagementPage() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setStaff(staffData);
      setUsers(allUsers.filter((user) => !user.deleted && (user.role === "USER" || !user.roles?.length)));
      setRoles(roleData);
  ...
  const selectedUsers = staff.filter((user) => selectedIds.includes(user.id) && !user.deleted);
  ...
  const columns = [
    ...
    { key: "fullName", title: "Nhân sự", render: (row) => <button className="font-semibold text-brand-700 hover:underline" onClick={() => setSelectedUser(row)}>{row.displayName || row.fullName}</button> },
    ...
    { key: "status", title: "Trạng thái", render: (row) => row.deleted ? <span className="font-semibold text-slate-500">Ngưng hoạt động</span> : row.active ? <span className="font-semibold text-emerald-600">Hoạt động</span> : <span className="font-semibold text-rose-600">Bị khóa</span> },
    { key: "action", title: "Thao tác", render: (row) => <div className="flex gap-2">{canAssign ? <Button size="sm" onClick={() => setAssigningUser(row)}>Nâng cấp</Button> : null}{canLock && !row.deleted ? <Button size="sm" variant={row.active ? "danger" : "secondary"} onClick={async () => { try { await handleToggleStatus(row.id); toast.success("Cập nhật trạng thái thành công"); load(); } catch (error) { toast.error(error?.message || "Khóa/mở khóa thất bại"); } }}>{row.active ? "Khóa" : "Mở khóa"}</Button> : null}{canDelete && !row.deleted ? <Button size="sm" variant="outline" onClick={async () => { if (!window.confirm(`Ngưng hoạt động tài khoản "${row.displayName || row.fullName}"?`)) return; try { await handleDeleteUser(row.id); toast.success("Đã ngưng hoạt động tài khoản"); load(); } catch (error) { toast.error(error?.message || "Xóa người dùng thất bại"); } }}>Ngưng</Button> : null}{row.deleted ? <span className="text-sm text-slate-400">Đã ngưng</span> : null}{!row.deleted && !canAssign && !canLock && !canDelete ? <span className="text-sm text-slate-400">Không có quyền</span> : null}</div> },
  ];
  ...
  const runBulkDelete = async () => {
    if (!selectedUsers.length) return;
    if (!window.confirm(`Ngưng hoạt động ${selectedUsers.length} nhân sự đã chọn?`)) return;
    try { for (const user of selectedUsers) await handleDeleteUser(user.id); toast.success("Đã ngưng hoạt động các tài khoản đã chọn"); setSelectedIds([]); load(); } catch (error) { toast.error(error?.message || "Xóa người dùng thất bại"); }
  };

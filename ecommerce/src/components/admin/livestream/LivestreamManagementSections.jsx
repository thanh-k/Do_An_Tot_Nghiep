...
export function LivestreamForm({ form, setForm, onSubmit, editMode, onCancel, onThumbnailChange, thumbnailPreview }) {
  return (
    <SectionCard
...
export function LivestreamList({ livestreams, selectedLive, onSelect, onCreate, onEdit, onDelete, canManage }) {
  return (
    <SectionCard
...
export function ProductPicker({ products, liveProducts, categories, brands, onAdd, onRemove, onClose }) {
...
export function AdminLiveChat({ liveId, broadcastLiveEvent, incomingMessage, canManage }) {
  ...
  useEffect(() => {
    ...
    // Không dùng scrollIntoView ở đây vì trang admin đang reload dữ liệu định kỳ.
    // scrollIntoView có thể kéo cả trang quản lý bị trượt xuống sau mỗi lần polling.
    // Chỉ cuộn bên trong khung bình luận khi thật sự có tin nhắn mới.
  ...
  return <div className="rounded-3xl border border-slate-200 p-4">
    ...
    <div ref={chatListRef} className="mt-3 h-56 overflow-y-auto rounded-2xl bg-slate-50 p-3 text-sm">
...
export function AdminLiveStudio({ live, onReload, onRemoveProduct, onOpenProductManager, canManage }) {

...
public class UserController {
    ...
    public ApiResponse<UserResponse> updateCurrentUser(@RequestBody UserUpdateRequest request) {
        return ApiResponse.<UserResponse>builder().message("Cập nhật hồ sơ thành công").result(userService.updateCurrentUser(request)).build();
    }
    ...
    public ApiResponse<UserResponse> updateAvatar(@RequestParam("file") MultipartFile file) throws IOException {
        return ApiResponse.<UserResponse>builder().message("Cập nhật ảnh đại diện thành công").result(userService.updateAvatar(file)).build();
    }

    @GetMapping("/users/me/addresses")
    public ApiResponse<List<UserAddressResponse>> getMyAddresses() {
        return ApiResponse.<List<UserAddressResponse>>builder().result(userService.getCurrentUserAddresses()).build();
    }
    ...
    public ApiResponse<UserAddressResponse> createAddress(@RequestBody UserAddressRequest request) {
        return ApiResponse.<UserAddressResponse>builder().message("Thêm địa chỉ thành công").result(userService.createCurrentUserAddress(request)).build();
    }
    ...
    public ApiResponse<UserAddressResponse> updateAddress(@PathVariable Long id, @RequestBody UserAddressRequest request) {
        return ApiResponse.<UserAddressResponse>builder().message("Cập nhật địa chỉ thành công").result(userService.updateCurrentUserAddress(id, request)).build();
    }
    ...
    public ApiResponse<String> deleteAddress(@PathVariable Long id) {
        ...
        return ApiResponse.<String>builder().message("Xóa địa chỉ thành công").result("OK").build();
    }

    @PatchMapping("/users/me/addresses/{id}/default")
    public ApiResponse<UserAddressResponse> setDefaultAddress(@PathVariable Long id) {
        return ApiResponse.<UserAddressResponse>builder().message("Đã chọn địa chỉ mặc định").result(userService.setDefaultAddress(id)).build();
    }
    ...
    public ApiResponse<UserResponse> adminUpdateUser(@PathVariable Long id, @RequestBody AdminUserUpdateRequest request) {
        return ApiResponse.<UserResponse>builder().message("Cập nhật người dùng thành công").result(userService.adminUpdateUser(id, request)).build();
    }

    @DeleteMapping("/admin/users/{id}")
    public ApiResponse<String> deleteUser(@PathVariable Long id) {
        userService.deleteUser(id);
        return ApiResponse.<String>builder().message("Xóa người dùng thành công").result("OK").build();
    }
}
